...
state.csv_file_name = get_unique_filename(generate_csv_name_with_config("macd"))
...
def main():
    ...
    for i in range(initial_candles, len(db_data)):
        ...
        df = calculate_macd(candles_to_check.copy())

        # enviar 100 velas
        signal, last_candle_checked = check_signals_macd(df)
